...
bound = [(0, 100) for _ in range(n_of_strats)]

# Constraints
const = {'type': 'eq', 'fun': lambda w: np.sum(w) - n}  # sum of all weight should be >= n_lower
...
bound = [(0, 100) for i in range(n_of_strats)] # no selling

const = {'type' : 'eq', 'fun' : lambda w : np.sum(w) - n} # sum of all weight is 15
...
w_list = np.arange(n_lower, n_higher)

res2_df = pd.DataFrame(columns = ['ratio', 'ratio_int'])

for i in w_list:

    const = {'type' : 'eq', 'fun' : lambda w : np.sum(w) - i} 
    opt_result = sciop.minimize(obj_function, x0 = weight, args = (df_pnl), bounds = bound, constraints = [const])

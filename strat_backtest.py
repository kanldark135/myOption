...
def get_trade_res(df, trade_dict: dict):
    ...
    if 'exit_date' in trade_dict.keys():
        df_trade_area = df.loc[trade_dict['entry_date'] : trade_dict['exit_date'], trade_dict['strikes']]
...
def get_return(df, dist_from_atm, number_of_contracts, preferred_weekday = 0, dte_range = [35,70]):

    res_list = []
    trade_list = generate_trade(df, dist_from_atm = dist_from_atm, number_of_contracts = number_of_contracts, preferred_weekday = preferred_weekday, dte_range = dte_range)

    for trade in trade_list:
        trade_res = get_trade_res(df, trade)['daily_ret']
...
def get_final_res(df_raw_subgroup, dist_from_atm, number_of_contracts, preferred_weekday = 0, dte_range = [35,70]):

    ''' 내가 저장해놓은 melt 된 데이터형태에서 필요한 값으로만 pivot 구성하기'''
    df = df_raw_subgroup.pipe(get_pivot_chain_within_group, values = ['price', 'iv', 'delta'])
    res = get_return(df, dist_from_atm = dist_from_atm, number_of_contracts = number_of_contracts)
    res = res['daily_ret'].cumsum()
    return res

# laddered strangle 1) at 0.05 delta 2) on monday 3) with iv dependent sizing 
...
if __name__ == "__main__":
    ...
    dist_from_atm = {'delta' : 0.1}
    number_of_contracts = [-1]
    preferred_weekday = 0
    dte_range = [35, 70]

    grouped = call_df_raw.groupby('expiry')

# 예시 : 2008-02-14 만기따리

    sample = grouped.get_group('2023-07-13')
    df = get_pivot_chain_within_group(sample)
    res = get_final_res(sample, dist_from_atm, number_of_contracts)

    trade_list = generate_trade(df, dist_from_atm, number_of_contracts)

    res0 = pd.concat([get_trade_res(df, trade_list[0])['df_premium'], get_trade_res(df, trade_list[0])['cum_ret']], axis = 1, join = 'inner')
    res1 = pd.concat([get_trade_res(df, trade_list[1])['df_premium'], get_trade_res(df, trade_list[1])['cum_ret']], axis = 1, join = 'inner')
    res2= pd.concat([get_trade_res(df, trade_list[2])['df_premium'], get_trade_res(df, trade_list[2])['cum_ret']], axis = 1, join = 'inner')
    res3 = pd.concat([get_trade_res(df, trade_list[3])['df_premium'], get_trade_res(df, trade_list[3])['cum_ret']], axis = 1, join = 'inner')
    
    ret = get_return(df, dist_from_atm, number_of_contracts)

#%%  실전 분석

result = grouped.apply(get_final_res, 

...
ㅇ) 콜풋 스큐 심할때 (콜개똥값) -> 풋 2개 매도 + 선물매도로 유사 수익 구조 구축하면서 풋 IV만 harvest

ㅇ) 스큐 플랫 or 변동성 자체가 낮을때는 양매도 대신 버터플라이나 스트래들 매도? 

ㅇ) 위클리 0DTE 당일매매 전략 (월/목에 실시)
    1) 역추세 크레딧 스프레드 : 과열권 크레딧 스프레드 + 50% 익절
    2) 로또매수 : 로또성 존나 매수?

ㅇ) 주가 및 변동성 역발상 전략 
    1) 반등 전략 : 기준이? : 주가 n% 떨어졌을때? / 기술적 지표 침체?
    2) 반락은 하지 말 것 (주가 고점에서의 콜매도는 머저리 짓)
    3) 변동성 역발상 베가전략?? : 볼 n% / (며칠) 에 걸쳐 떨어졌을때 다음날 반등 예상한 양매수?

ㅇ) dynamic 112 : 추가적인 management 방법론 정립
    - 외가풋만 -n% 수익에서 청산
    - 전량손절점
    - 풋 데빗 스프레드 수익실현 후 하향조정하는 시점

ㅇㅇ) BSH : 방법론 정립
    - 규모 : 개별 전략으로써가 아니라 다른 숏베가 전략들의 베가위험 및 감마위험 전부 cover 할수 있는 규모
    - 익절 : 대 폭락시 언제 수익실현?
    - (만일 애매하게 폭락하여 만기 직전까지 가만있는 경우) 데스밸리 손절점


V3) -----------------(완료) 백스프레드 위클리로 하는것도 테스트 -> incorporated
V4) -----------------(완료)감마 짤짤이 계열 (익절 + 손절 조정해가면서) 테스트
                            * 양매수로 테스트 했고 유의미한 전략 있으나, 크게 X
V8) -------------- (완료) 위클리로 콜매도 / 풋매도 방향성 매도 테스트 -> incorporated

...
3) -----------------(완료) 백스프레드 위클리로 하는것도 테스트

4) 감마 짤짤이 계열 (익절 + 손절 조정해가면서) 테스트 : 

* 현실적으로 원하는 가격에 매매짤짤이 가능한 행사가 및 만기로 제한 필요 (매매안되는 차월물 등가/내가 같은거 X)
    4-1)델타50 -1 25 매도-2 짜리 크레딧 풋 백스프레드 + 빠르게 청산 -> 위든 아래든 움직이기만 하면 수익
    : 양매수랑 다를게...?
    4-2) 아예 양매수 + 익절 빠르게 전략도 테스트

5) 위클리 0DTE 전략 (월/목에 실시)
    5-1) 과열권 크레딧 스프레드 + 50% 익절
    5-2) ?

6) 112 : tom king 자료 보면서 추가적인 management 방법론 정립
    - 외가풋만 -n% 수익에서 청산
    - 전량손절점
    - 풋 데빗 스프레드 수익실현 후 하향조정하는 시점

7) BSH : 방법론 정립
    - 개별 전략으로써가 아니라 다른 숏베가 전략들의 베가위험 및 감마위험 전부 cover 할수 있는 수준의 수량 도출 
    - 수익실현점
    - (만일 애매하게 폭락하여 만기 직전까지 가만있는 경우) 데스밸리 손절점

8) -------------- (완료) 위클리로 콜매도 / 풋매도 방향성 매도 테스트
9) IV 낮을때 구축할만한 매수전략 없나?

...
ㅇ) 콜이 풋대비 너무 낮을때 -> 풋 2개 매도 + 선물매도로 유사 수익 구조 구축하면서 풋 IV만 harvest
...
V) -----------------(완료) 백스프레드 위클리로 하는것도 테스트 -> incorporated
V) -----------------(완료)감마 짤짤이 계열 (익절 + 손절 조정해가면서) 테스트
                            * 양매수로 테스트 했고 유의미한 전략 있으나, 크게 X
V) -------------- (완료) 위클리로 콜매도 / 풋매도 방향성 매도 테스트 -> incorporated

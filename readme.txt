...
5) 추가수익원1 : 위클리 0DTE 전략 (월/목에 실시)
    - 장중 과열권에서 반전 크레딧 스프레드 + 50% 익절
    - ?

6) 스큐전략 (스큐 많이 낮을때) or 변동성 낮을때 용으로 버터플라이 고민

7) 저가매수 전략(상승했을때 반락은 XX) 
    - 현재 60일 볼린저 터치
    - +주가 n% 떨어졌을때 반등?
    
7) 변동성 너무 높아졌을때 전략

8) dynamic 112 : tom king 자료 보면서 추가적인 management 방법론 정립
    - 외가풋: 1) 둘 다 95% 수익에서 청산 / 2) 한개만 청산
    - 전량손절점
...
V3) -----------------(완료) 백스프레드 위클리로 하는것도 테스트

V4) -----------------(완료)감마 짤짤이 계열 (익절 + 손절 조정해가면서) 테스트
                            * 양매수로 테스트 했고 유의미한 전략 있으나, 크게 X

V8) -------------- (완료) 위클리로 콜매도 / 풋매도 방향성 매도 테스트
